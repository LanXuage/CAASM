...
logger = logging.getLogger("shared.nebula")
...
def _get_all_annotations(model: Type[T]) -> Dict[str, Any]:
    """Collect annotations from the model class and all its parent classes."""
...
def make_object(model: Type[T], vertex: Vertex, **kwargv: Any) -> T:
    annotations = _get_all_annotations(model)
