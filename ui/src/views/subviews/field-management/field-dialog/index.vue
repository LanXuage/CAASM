...
import type { FieldCollect } from '../../../../api/field-collect/types'
import type { CheckboxValueType } from 'element-plus'
import { cloneDeep } from 'lodash'
import { fieldCollectApi } from '../../../../api'
...
const field = defineModel<Field>("field", { default: { fieldName: '', fieldDesc: '', collects: [] } })
const emit = defineEmits<{ submit: [field: Field] }>()

const fieldCollects = reactive<Array<FieldCollect>>([])
...
const fieldForm = reactive<Field>(cloneDeep(field.value))
const fieldRules = reactive<FormRules<Field>>({
    fieldName: [
...
const refreshFieldCollects = () => {
    ...
    fieldCollectApi.getFieldCollects().then((res) => {
                ...
                <el-select v-model="fieldForm.collects" filterable multiple collapse-tags collapse-tags-tooltip
                    ...
                    <template #header>
                        <el-row style="display: flex; flex-direction: row; justify-content: space-between;">
                            ...
                            <el-button type="primary" :loading="refreshFieldCollectsLoading" :loading-icon="Refresh"
                                :icon="Refresh" link @click="refreshFieldCollects" />
                        </el-row>
                    </template>
                    <el-option v-for="item, i in fieldCollects" :key="i" :label="item.collectName"
